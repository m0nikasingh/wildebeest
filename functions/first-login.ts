...
export const onRequestPost: PagesFunction<Env, any, ContextData> = async ({ request, env, data }) => {
	return handlePostRequest(env.KV_CACHE, request, env.DATABASE, env.userKEK)
}
...
export async function handlePostRequest(
	...
	const person = await createPerson(domain, db, userKEK, email, properties)

	if (!url.searchParams.has('redirect_uri')) {
		return new Response('', { status: 400 })
	}

	await session.storeUserSession(cache, email, person)
